...
public class EntityCow extends EntityAnimal
{
    ...
    public void onLivingUpdate()
    {
        ...
        {
            int var1 = this.worldObj.getBlockId(MathHelper.floor_double(this.posX), MathHelper.floor_double(this.boundingBox.minY) - 1, MathHelper.floor_double(this.posZ));

            if (var1 == Block.mycelium.blockID && this.worldObj.rand.nextInt(12000) == 0 && !(this instanceof EntityMooshroom))
            {
                this.setDead();
                this.worldObj.spawnParticle("largeexplode", this.posX, this.posY + (double)(this.height / 2.0F), this.posZ, 0.0D, 0.0D, 0.0D);
                EntityMooshroom var2 = new EntityMooshroom(this.worldObj);
                var2.setLocationAndAngles(this.posX, this.posY, this.posZ, this.rotationYaw, this.rotationPitch);
                var2.setEntityHealth(this.getHealth());
                var2.renderYawOffset = this.renderYawOffset;
                var2.setGrowingAge(this.getGrowingAge());
                this.worldObj.spawnEntityInWorld(var2);
                this.worldObj.playSoundAtEntity(var2, "mob.slimeattack", 1.0F, (this.rand.nextFloat() - this.rand.nextFloat()) * 0.2F + 1.0F);
                this.worldObj.playSoundAtEntity(var2, this.getHurtSound(), this.getSoundVolume(), (this.rand.nextFloat() - this.rand.nextFloat()) * 0.2F + 1.0F);
            }
        }
    }
}
