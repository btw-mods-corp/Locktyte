...
public class BlockFire extends Block
{
    ...
    public void updateTick(World par1World, int par2, int par3, int par4, Random par5Random)
    {
        ...
        if (par1World.getBlockId(par2, par3 - 1, par4) == mod_FCBetterThanWolves.fcBBQ.blockID)
        {
            if (!((FCBlockBBQ)mod_FCBetterThanWolves.fcBBQ).IsBBQLit(par1World, par2, par3 - 1, par4))
            {
                par1World.setBlockWithNotify(par2, par3, par4, 0);
                return;
            }
        ...
        else if (par1World.getBlockId(par2, par3 - 1, par4) == mod_FCBetterThanWolves.fcStokedFire.blockID)
        {
            var6 = true;
        }
    ...
    protected void tryToCatchBlockOnFire(World par1World, int par2, int par3, int par4, int par5, Random par6Random, int par7)
    {
        int var8 = this.abilityToCatchFire[par1World.getBlockId(par2, par3, par4)];

        if (par6Random.nextInt(par5) < var8)
        {
            this.OnBlockDestroyedByFire(par1World, par2, par3, par4);
     ...
     */
    protected int getChanceOfNeighborsEncouragingFire(World par1World, int par2, int par3, int par4)
    {
        byte var5 = 0;
    ...
    protected void OnBlockDestroyedByFire(World var1, int var2, int var3, int var4)
    {
        int var5 = var1.getBlockId(var2, var3, var4);

        if (var5 == mod_FCBetterThanWolves.fcCompanionCube.blockID)
        {
            FCBlockCompanionCube.SpawnHearts(var1, var2, var3, var4);
        }
        else if (var5 == mod_FCBetterThanWolves.fcMiningCharge.blockID)
        {
            FCBlockMiningCharge.CreatePrimedEntity(var1, var2, var3, var4, var1.getBlockMetadata(var2, var3, var4));
        }
        else if (var5 == mod_FCBetterThanWolves.fcLeaves.blockID)
        {
            mod_FCBetterThanWolves.fcLeaves.dropBlockAsItem(var1, var2, var3, var4, var1.getBlockMetadata(var2, var3, var4), 0);
        }
    }

    public boolean CanBlockBeDestroyedByFire(int var1)
    {
        return this.abilityToCatchFire[var1] > 0;
    }
