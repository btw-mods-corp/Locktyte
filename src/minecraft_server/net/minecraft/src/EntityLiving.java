...
public abstract class EntityLiving extends Entity
{
    ...
    protected void onDeathUpdate()
    {
        ...
        if (this.deathTime == 20)
        {
            ...
            else if (!this.worldObj.isRemote && !this.isChild())
            {
                ...
                if (var1 > 0)
                {
                    this.worldObj.spawnEntityInWorld(new EntityXPOrb(this.worldObj, this.posX, this.posY, this.posZ, this.getExperiencePoints(this.attackingPlayer), true));
                }
            }
    ...
    public void onDeath(DamageSource par1DamageSource)
    {
        ...
        if (!this.worldObj.isRemote)
        {
            ...
            if (!this.isChild())
            {
                this.dropFewItems(true, var3);
                int var4 = this.rand.nextInt(800) - var3 * 4;

                if (var4 < 5)
                {
                    this.dropRareDrop(var4 > 0 ? 0 : 1);
                }
    ...
    public void moveEntityWithHeading(float par1, float par2)
    {
        ...
        else
        {
            ...
            float var8 = 0.1627714F / (var3 * var3 * var3);
            float var5;
    ...
    public void writeEntityToNBT(NBTTagCompound par1NBTTagCompound)
    {
        ...
        if (!this.activePotionsMap.isEmpty())
        {
            ...
            Iterator var4 = this.activePotionsMap.values().iterator();

            while (var4.hasNext())
            {
                PotionEffect var5 = (PotionEffect)var4.next();
                NBTTagCompound var3 = new NBTTagCompound();
                var3.setByte("Id", (byte)var5.getPotionID());
                var3.setByte("Amplifier", (byte)var5.getAmplifier());
                var3.setInteger("Duration", var5.getDuration());
                var2.appendTag(var3);
            }
    ...
    public void onLivingUpdate()
    {
        ...
        if (this.isJumping)
        {
            if ((!this.isInWater() || this.isWeighted()) && (!this.handleLavaMovement() || this.isWeighted()))
            {
    ...
    public Vec3 getLook(float par1)
    {
        ...
        if (par1 == 1.0F)
        {
            var2 = MathHelper.cos(-this.rotationYaw * 0.01745329F - (float)Math.PI);
            var3 = MathHelper.sin(-this.rotationYaw * 0.01745329F - (float)Math.PI);
            var4 = -MathHelper.cos(-this.rotationPitch * 0.01745329F);
            var5 = MathHelper.sin(-this.rotationPitch * 0.01745329F);
        ...
        else
        {
            ...
            var4 = MathHelper.cos(-var3 * 0.01745329F - (float)Math.PI);
            var5 = MathHelper.sin(-var3 * 0.01745329F - (float)Math.PI);
    ...
    public void CheckForScrollDrop()
    {
        if (this.worldObj.getWorldInfo().getGameType() != EnumGameType.CREATIVE)
        {
            ...
            if (this.rand.nextInt(250) == 0)
            {
                ...
                {
                    var1 = Enchantment.knockback.effectId;
                }
                else if (this.rand.nextInt(2) == 0)
                {
                    if (this instanceof EntityGhast)
                    {
                        var1 = Enchantment.flame.effectId;
                    }
                    else if (this.rand.nextInt(2) == 0)
                    {
                        if (this instanceof EntityCreeper)
                        {
                            var1 = Enchantment.blastProtection.effectId;
                        }
                        else if (this instanceof EntityZombie)
                        {
                            var1 = Enchantment.smite.effectId;
                        }
                        else if (this instanceof EntitySpider)
                        {
                            var1 = Enchantment.baneOfArthropods.effectId;
                        }
                        else if (this instanceof EntityEnderman)
                        {
                            var1 = Enchantment.silkTouch.effectId;
                        }
                    }
                }
            }
            ...
            if (var1 >= 0)
            {
                ItemStack var2 = new ItemStack(mod_FCBetterThanWolves.fcArcaneScroll, 1, var1);
