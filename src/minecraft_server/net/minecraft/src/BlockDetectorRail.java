...
public class BlockDetectorRail extends BlockRail
{
    ...
    private void setStateIfMinecartInteractsWithRail(World par1World, int par2, int par3, int par4, int par5)
    {
        ...
        if (var9 != null && !var9.isEmpty())
        {
            for (int var10 = 0; var10 < var9.size(); ++var10)
            {
                ...
                if (ShouldPlateActivateBasedOnMinecart(par1World, par2, par3, par4, var11.minecartType, var11.riddenByEntity))
                {
                    var7 = true;
                    break;
                }
    ...
    public static boolean ShouldPlateActivateBasedOnMinecart(World var0, int var1, int var2, int var3, int var4, Entity var5)
    {
        int var6 = var0.getBlockId(var1, var2, var3);

        if (var6 == mod_FCBetterThanWolves.fcDetectorRailWood.blockID)
        {
            return true;
        }
        else
        {
            if (var6 == mod_FCBetterThanWolves.fcDetectorRailObsidian.blockID)
            {
                if (var5 != null && var5 instanceof EntityPlayer)
                {
                    return true;
                }
            }
            else if (var6 == Block.railDetector.blockID && (var4 > 0 || var5 != null))
            {
                return true;
            }
